				...
				function() {
					...
					$("#product-input").keyup(
							function(e) {
								...
								$.fn.filter(this, '#ul-product-tags', keyCode,
										'product');
							});
										...
										$(document)
												.click(
														function(e) {
															...
															var cl = $(e.target)
																	.closest(
																			'#ul-kbase-tags').length
															var l = cl.length;
															if ($(e.target)
																	.closest(
																			'#ul-kbase-tags').length != 0
																	|| $(
																			e.target)
																			.closest(
																					'#kbase-selection').length)
																return false;
															$('#kbase-tags')
																	.hide();
															$(document).unbind(
										...
										$(document)
												.click(
														function(e) {
															...
															var cl = $(e.target)
																	.closest(
																			'#ul-prodct-tags').length
															var l = cl.length;
															if ($(e.target)
																	.closest(
																			'#ul-prodct-tags').length != 0
																	|| $(
																			e.target)
																			.closest(
																					'#product-selection').length)
																return false;
															$('#product-tags')
																	.hide();
															$(document).unbind(
										...
										$(document)
												.click(
														function(e) {
															...
															var cl = $(e.target)
																	.closest(
																			'#ul-region-tags').length
															var l = cl.length;
															if ($(e.target)
																	.closest(
																			'#ul-region-tags').length != 0
																	|| $(
																			e.target)
																			.closest(
					...
					$.fn.addFilter = function(name, ulList) {
						...
						$('#searchlist').html(
								'<ul style="color: #000000; cursor: pointer;">'
										+ $(ulList).html() + '</ul>');
					...
					$.fn.showPopup = function(divValue, ulValue) {
						var docHeight = $(document).height(); // grab the
																// height of the
																// page
						var scrollTop = $(window).scrollTop(); // grab the px
																// value from
																// the top of
																// the page to
																// where you're
					...
					$.fn.filter = function(element, what, keyCode, type) {
						...
						log(value);
						log(keyCode);
						value = value.toLowerCase().replace(/\b[a-z]/g,
								function(letter) {
					...
					$.fn.addToSearchCloud = function(type, element) {
							...
							$('.ul_all_tags li').each(
									function(index) {
										...
										if (topicTag == 'sc-kbase') {
											...
											$.fn.removeTag(type, li.context.id
													.substring(3));
										}
									});
						...
						$(".dpui-widget").trigger("dpui:updateSearchCloud",
								buildLi);
					}
					...
					$.fn.parseTags = function(data) {
						if (typeof data.tagSets != 'undefined'
								&& data.tagSets != null
								&& data.tagSets.length > 0) {
							for (var x = 0; x < data.tagSets.length; x++) {
								if (data.tagSets[x].systemTagName === 'topic') {
									if (typeof data.tagSets[x].tags != 'undefined'
										...
										// var treeData =
										// $.fn.createTreeFilter(data.tagSets[x].tags,
										// 'topic_westjet');
										var treeData = $.fn.createTreeFilter(
												data.tagSets[x].tags, 'topic',
												'topic', false);
										// $('#div-topic-tags').html(treeData);
									}
									...
									if (typeof data.tagSets[x].tags != 'undefined'
										...
										// var treeData =
										// $.fn.createTreeFilter(data.tagSets[x].tags,
										// 'product_westjetproducts');
										var treeData = $.fn.createTreeFilter(
												data.tagSets[x].tags,
												'product', 'product', false);
										$('#div-product-tags').html(treeData);
										$('#ul-product-tags').html(
												$.fn.setupFilter(
														data.tagSets[x].tags,
														'product', 'product'));
									}
									...
									if (typeof data.tagSets[x].tags != 'undefined'
											...
											&& data.tagSets[x].tags.length > 0) {
										var contentData = $.fn
												.setupContentFilter(data.tagSets[x].tags);
										$('#div-content-tags')
												.html(contentData);
										$(".dpui-widget").trigger(
												"dpui:setupContentTypeTags",
												$.fn.getSelectedContentTags());
									}
								}
							}
					...
					$.fn.populateCrossTags = function(data) {
						...
						for (var i = 0; i < data.crossTags.length; ++i) {
							...
							if (currentTargetSet == 'topic') {
								...
								// types
								var index = crossTagTypes
										.indexOf(currentTargetSet);
								if (index > -1) {
					...
					$.fn.populateKBaseTags = function(data) {
						...
						if (typeof data.tags != 'undefined'
							...
							for (var x = 0; x < data.tags.length; x++) {
								if (typeof data.tags[x] != 'undefined'
										&& data.tags[x] != null
										&& data.tags[x].systemTagName.length > 0) {
									$('#div-kbase-tags').html(
							...
							PARAMSLOOP: for (var i = 0; i < parameterArray.length; ++i) {
								for (var j = 0; j < data.tags.length; ++j) {
									if (parameterArray[i] == data.tags[j].systemTagName) {
										...
										var elem = document
												.getElementById(data.tags[j].systemTagName);
							...
							if (noKBaseInParameters) {
								if (data.tags[0] != 'undefined') {
									...
									// applies the first kbase parameter that
									// gets
									var elem = document
					...
					$.fn.manageTheme = function() {
						...
						if (current != 'undefined' && current != null) {
							...
							if ($.fn
									.hasCommonElement(currentArray, configArray)) {
								document
										.getElementById('knowledgeCentralTheme').disabled = false;
								document.getElementById('defaultTheme').disabled = true;
							} else {
								document
										.getElementById('knowledgeCentralTheme').disabled = true;
