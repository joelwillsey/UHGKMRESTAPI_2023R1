...
# Path the em-appserver, export only KM_MW_HOME%JAVA_WLST_OPTIONS%
...
export KM_STARTUP_OPTIONS= -Denvironment.name=local -DconfigLocation=/app_2/verint/em/projects/uhg/KMRestAPI/kmservices/config -Dmachine.name=localhost -Dcontainer.name=prod -DlogFile=/app_2/verint/em/logs
export KM_MEMORY_OPTS= -Xms1024m -Xmx1024m

export ORIGINAL_JAVA_OPTIONS = $JAVA_OPTIONS
# export this variable as a check for the other scripts if the all config and environment variables have been exported
